...
rm(estres, players21, dato_calculo.1,profesores2)   # eliminar datas no necesarias
...
profesores2 <- rename(profesores, fecha= Fecha, edad=Edad, sexo=Genero, niveldocencia= Nivel_docencia, 
...
indicador_2a <- select(profesores2, 2, 3, 4, 6)
...
profesores2 %>% 
  filter(sexo=="Femenino" & (edad == c(28,29,30))) %>% 
  select(edad, alumnos, niveldocencia, material) %>% 
  view()
...
profesores2 %>% 
  mutate(diaencuesta=day(fecha)) %>% 
  filter(sexo=="Masculino" & (edad %in% c(30,31,32))) %>% 
  select(c(1:7, diaencuesta, 14)) %>% 
  view()

profesores2 %>% 
  mutate(mesencuesta=months(fecha)) %>% 
  filter(sexo=="Masculino" & (edad %in% c(30,31,32))) %>% 
  select(c(1:7, mesencuesta, 14)) %>% 
  view()
...
profesores2 %>% 
  summarise(EstudiantesPromedio = mean(alumnos)) %>% 
  view()
...
profesores2 %>% 
  ...
  summarise(Profesores = n(),
            ...
            Estudiantes_Promedio = round(mean(alumnos)),
            Edad_Promedio = round(mean(edad))) %>%
